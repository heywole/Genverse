...
import { Home, Compass, PlusCircle, BookOpen, Users, FolderOpen, ShieldCheck, ChevronLeft, ChevronRight, ChevronDown, Trophy } from 'lucide-react'
...
export function Sidebar({ session: initialSession }: Props) {
  ...
  // Sync collapsed state to a CSS class on <html> so the spacer can respond
  useEffect(() => {
    if (collapsed) {
            ...
            {!collapsed && (
              <>
                ...
                <ChevronDown
                  size={12}
                  style={{
                    transform: buildersOpen ? 'rotate(180deg)' : 'rotate(0deg)',
